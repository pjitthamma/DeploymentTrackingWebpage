...
$StartDate = getArrayResult("SELECT DeploymentDate from DeploymentRecord order by RecordId ASC LIMIT 1;")[0]["DeploymentDate"];
$EndDate = getArrayResult("SELECT DeploymentDate from DeploymentRecord order by RecordId DESC LIMIT 1;")[0]["DeploymentDate"];
$weekCount = 1;
...
while (strtotime($weekMonday) <= strtotime("today")) {
  ...
  $query = "select count(*) as times from DeploymentRecord where DeploymentDate >= '".$weekMonday."' and DeploymentDate <= '".$weekFriday."' and ReleaseStatus = 2;";
  $FailedRelease = getArrayResult($query)[0]["times"];

//count success each day
  $query = "select distinct DeploymentDate, (select count (*) from DeploymentRecord where DeploymentDate = main.DeploymentDate and ReleaseStatus = '1') as times from DeploymentRecord main where DeploymentDate >='".$weekMonday."'and DeploymentDate <= '".$weekFriday."'";
  
  $resultSuccessCount = getArrayResult($query);

  $SuccessRelease = 0;
  // loop query
  foreach($resultSuccessCount as $result){
    $SuccessRelease += $result["times"];
  }

  $query = "select count(*) as times from DeploymentRecord where DeploymentDate >= '".$weekMonday."' and DeploymentDate <= '".$weekFriday."' and LiveRollBack = 2;";
  $Rollback = getArrayResult($query)[0]["times"];

// count row of week
  $query = "select distinct DeploymentDate, (select count (*) from DeploymentRecord where DeploymentDate = main.DeploymentDate) as times from DeploymentRecord main where DeploymentDate >= '".$weekMonday."' and DeploymentDate <= '".$weekFriday."'";
  $rowCount = count(getArrayResult($query));

  $query = "select count(*) as times from DeploymentRecord where DeploymentDate >= '".$weekMonday."' and DeploymentDate <= '".$weekFriday."' and ReleaseStatus != 0;";
  $Total = getArrayResult($query)[0]["times"];

  if ($Total != 0) 
  {
    $SuccessRatio = number_format((float)(($SuccessRelease*100)/($rowCount*2)),'2','.','');
  }else {
    $SuccessRatio = 0;
  }
...
<head>
  ...
  google.load('visualization', '1', {packages: ['corechart', 'linechart']});
  ...
  function drawLineChart() {
    ...
    var options = {
      title: 'Website Deployment Success Ratio',
...
<body>
  ...
  <div class="container" id="center">
    <h2>FE Deployment Summary : <small><?php echo date("j M Y" ,strtotime("today")); ?></small></h2>
    <div class="row">
    </div>
    <div class="row">
      <div class="col-md-3">
        <div id="success_ratio">
          <table class="table table-condensed table-bordered">
            <thead class="bg-primary">
              <th class="vert-align text-center">THIS WEEK KPI SUCCESS RATIO</th>
            </thead>
      ...
      <div class="col-md-6" id="center" style="text-align:center;padding-top:40px;">
      <span>
        <a class="btn btn-success" href="dailyReport_with_KPI.php" disabled>Base on Deployment KPI</a></span>
        <a class="btn btn-success" href="dailyReport.php" >Base on Total packages</a>
      </div>
      ...
      <div id = "minitable" class="col-md-3">
        <table class="table table-condensed table-bordered">
          ...
          <tr class="bg-danger">
            <td class="text-center" colspan="2"><b>Website Deployment KPI is <br> <font color="green">2 success packages per day</font><b></td>
          </tr>
