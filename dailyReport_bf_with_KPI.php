...
$StartDate = getArrayResult("SELECT DeploymentDate from DeploymentRecord_bf order by RecordId ASC LIMIT 1;")[0]["DeploymentDate"];
$EndDate = getArrayResult("SELECT DeploymentDate from DeploymentRecord_bf order by RecordId DESC LIMIT 1;")[0]["DeploymentDate"];
$weekCount = 1;
...
while (strtotime($weekMonday) <= strtotime("today")) {
  ...
  $query = "SELECT count(*) as times from DeploymentRecord_bf where DeploymentDate >= '".$weekMonday."' and DeploymentDate <= '".$weekFriday."' and ReleaseStatus = 2;";
  $FailedRelease = getArrayResult($query)[0]["times"];

//count success each day
  $query = "select distinct DeploymentDate, (select count (*) from DeploymentRecord_bf where DeploymentDate = main.DeploymentDate and ReleaseStatus = '1') as times from DeploymentRecord_bf main where DeploymentDate >='".$weekMonday."'and DeploymentDate <= '".$weekFriday."'";
  ...
  $query = "SELECT count(*) as times from DeploymentRecord_bf where DeploymentDate >= '".$weekMonday."' and DeploymentDate <= '".$weekFriday."' and LiveRollBack = 2;";
  $Rollback = getArrayResult($query)[0]["times"];

// count row of week
  $query = "select distinct DeploymentDate, (select count (*) from DeploymentRecord_bf where DeploymentDate = main.DeploymentDate) as times from DeploymentRecord_bf main where DeploymentDate >= '".$weekMonday."' and DeploymentDate <= '".$weekFriday."'";
  $rowCount = count(getArrayResult($query));

  $query = "SELECT count(*) as times from DeploymentRecord_bf where DeploymentDate >= '".$weekMonday."' and DeploymentDate <= '".$weekFriday."' and ReleaseStatus != 0;";
  $Total = getArrayResult($query)[0]["times"];

  if ($Total != 0) 
  {
    $SuccessRatio = number_format((float)(($SuccessRelease*100)/($rowCount*1)),'2','.','');
  }else {
    $SuccessRatio = 0;
  }
...
<head>
  ...
  function drawLineChart() {
    ...
    var options = {
      title: 'Booking Form Deployment Success Ratio',
      bar: {groupWidth: 20},
      legend: {
        position: 'none'
      },
...
<body>
  ...
  <div class="container" id="center">
    <h2>BF Deployment Summary : <small><?php echo date("j M Y" ,strtotime("today")); ?></small></h2>
    <div class="row">
    </div>
    <div class="row">
      <div class="col-md-3">
...
<div class="col-md-6" id="center" style="text-align:center;padding-top:40px;">
      <span>
        ...
        <a class="btn btn-success" href="dailyReport_bf.php" >Base on Total packages</a>
      </div>
      <div id = "minitable" class="col-md-3">
        <table class="table table-condensed table-bordered">
          ...
          <tr class="bg-danger">
            <td class="text-center" colspan="2"><b>Booking form Deployment KPI is <br> <font color="green">1 success packages per day</font><b></td>
          </tr>
          <tr class="bg-info">
            <td>FAIL</td>
